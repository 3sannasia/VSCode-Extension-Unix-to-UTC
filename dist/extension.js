...
const node_child_process_1 = __webpack_require__(2);
...
async function convertTime(context) {
    let convertTimeCommand = vscode.commands.registerCommand("unix-to-utc.convertTime", async () => {
        ...
        if (editor) {
            ...
            const response = await fetch(`http://127.0.0.1:8001/convert/${text.toString()}`);
            if (!response.ok) {
...
async function replaceTime(context) {
    let replaceTimeCommand = vscode.commands.registerCommand("unix-to-utc.replaceTime", async () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            const document = editor.document;
            const selection = editor.selection;
...
async function getUTCTime(context) {
    let getUTCTimeCommand = vscode.commands.registerCommand("unix-to-utc.getUTCTime", async () => {
        const response = await fetch("http://127.0.0.1:8001/current_utc_timestamp");
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status}`);
        }
        const data = await response.json();
...
async function getUnixTime(context) {
    let getUnixTimeCommand = vscode.commands.registerCommand("unix-to-utc.getUnixTime", async () => {
        const response = await fetch("http://127.0.0.1:8001/current_unix_timestamp");
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status}`);
        }
        const data = await response.json();
...
function activate(context) {
    ...
    api_process = (0, node_child_process_1.exec)("python3 " + path_1.default.join(__dirname, "datetime_api.py"), (error, stderr) => {
        if (error) {
